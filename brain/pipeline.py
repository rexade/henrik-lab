...
async def summarize_chunk(text: str, n: int, total: int, source: str) -> str:
    msg = await ai.messages.create(
        ...
        max_tokens=4096,
        messages=[{"role": "user", "content":
            f"Summarize chunk {n}/{total} from '{source}'. "
            f"Preserve ALL key facts, concepts, arguments, data, and actionable info. "
            f"Be thorough.\n\n{text}"
        }],
    )
    return msg.content[0].text
...
DISTILL_PROMPT = """\
You are a knowledge distillation expert. Transform this content into a clean structured document.
...
Respond with ONLY valid JSON:
{
  ...
  "tags": ["tag1", "tag2", "tag3"],
  "tldr": "2-4 sentence summary of the entire content",
  "key_points": ["point 1", "point 2", "point 3"],
  "clean_markdown": "full clean markdown with sections: ## TL;DR\\n## Key Points\\n## Details\\n## How to Apply (if relevant)\\n## Sources"
}
